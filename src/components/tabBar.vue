<template>
    <div :class="['wrapper', isIpx&&isIpx()?'w-ipx':'']">
        ...
        <div class="bar-item" @click="tabTo('score')">
            <text class="bar-ic iconfont" :class="[this.pIndexKey == 'score'?'bar-active':'']">&#xe744;</text>
            <text class="bar-txt" :class="[this.pIndexKey == 'score'?'bar-active':'']">比分</text>
            <!-- <text class="notice-dot"></text> -->
        </div>
        <div class="bar-item" @click="tabTo('club')">
            <text class="bar-ic iconfont" :class="[this.isActive('club')]">&#xe605;</text>
            <text class="bar-txt" :class="[this.isActive('club')]">俱乐部</text>
        </div>
        <div class="bar-item" @click="tabTo('find')">
            <text class="bar-ic iconfont" :class="[this.isActive('find')]">&#xe61a;</text>
            <text class="bar-txt" :class="[this.isActive('find')]">发现</text>
        </div>
